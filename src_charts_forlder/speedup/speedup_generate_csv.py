...
for file_path in data_files:
    ...
    for _, row in df.iterrows():
        ...
        if time_serial is not None and time_parallel > 0:
            ...
            results.append([name_matrix, num_threads, computation_type, speedup])

# Creare il DataFrame con i risultati
output_df = pd.DataFrame(results, columns=["name_matrix", "num_threads", "computation_type", "speedup"])
